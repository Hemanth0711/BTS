...
// Add event listener to each list item to toggle selection
...
function toggleDeleteButton() {
  let selectedItems = ulEl.querySelectorAll(".selected");
  let deleteText = selectedItems.length > 0 ? "Delete" : "Delete All";
  deleteEl.textContent = deleteText;
}
...
deleteEl.addEventListener("click", function() {
  ...
  let savedTabs = JSON.parse(localStorage.getItem("savedTabs"));
  let updatedTabs = savedTabs.filter((tab) => {
    return !selectedItems.some((item) => {
      return item.textContent === tab;
    });
  });
